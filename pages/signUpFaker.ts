...
export class SignUpFaker extends CommonElementsForAccess{
...
async fillValuesSignUpForm (password: string, repeatPassword: string, accept: boolean, subsribe: boolean){

    const nameField = faker.person.firstName();
    const emailAddressField = faker.internet.email();

    await this.signUpNameField.fill(nameField);
    await expect (this.signUpNameField).toHaveValue(nameField);

    await this.commonEmailField.fill(emailAddressField);
    await expect (this.commonEmailField).toHaveValue(emailAddressField);
